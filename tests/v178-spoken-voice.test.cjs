...
(async () => {
  ...
  /* ============ 3. one voice, both writers ============ */
  {
    ...
    assert.ok(/scriptPrompt\(topic, option, fmt, await voiceBrief\(\), await readAbout\(\)\)/.test(HOOKS_LIB),
      "the SCRIPT writer goes through it — and since v205 carries the playbook alongside it");
